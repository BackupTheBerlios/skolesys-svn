...
from skolesys.lib.hostmanager import check_hostname,check_hosttype_text
...
if os.path.exists('./skolesoap.conf'):
	...
	soapconf = conf2dict('/etc/skolesys/skolesoap.conf')
        ...
        print "skolesoap.conf could be read. In order to register this host you must deliver the mainserver SOAP url and port number."
...
while not options.hostname:
	...
	if not options.hostname:
		print '"%s" is not a valid hostname. Use only letters and numbers.'
